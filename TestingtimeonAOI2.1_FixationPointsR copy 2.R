...
ptime <- system.time({dFF<-cbind(cbind(EveryAOItimeStatL(incorrectstatsFF),
                                       EveryAOItimeStatR(incorrectstatsFF)),dFF)})
dFFdFS<-cbind(cbind(EveryAOItimeStatL(incorrectstatsFS),EveryAOItimeStatR(incorrectstatsFS)),dFS)
...
saveFile <- paste(getwd(), "Master RDA files_RG/Heatmapdata_FixationPoints.rda", sep = "/")
save(dFFTyp,dFSTyp,dSFTyp,dITyp, dFFITyp,dFSITyp, dSFITyp, dIITyp, dFFCTyp,dFSCTyp,dSFCTyp,dICTyp,dFFASD,dFSASD,dSFASD,dIASD,dFFCASD,dFSCASD,dSFCASD,dICASD,dFFIASD,dFSIASD,dSFIASD,dIIASD, file = paste(getwd(),"/Master RDA files_RG/Heatmapdata_FixationPoints.rda",sep=""))
...
writetoExcelone(dfNoNas, paste(getwd(),"/Condition CSV files_RG/aggregate_data_filtered_all2_FixationPoint.xlsx",sep=""), "aggregate_data", "table3")
writetoCSVone(df, "aggregate_data_all_FixationPoint")
writetoCSVone(df2, "aggregate_data_incorrect_FixationPoint")
writetoCSVone(df3, "aggregate_data_correct_FixationPoint")

df <- rbind(dFFITyp, dFFIASD)
dfNoNas <- replaceNAWithMeans(df)
list <- unlist(lapply(rownames(dfNoNas), function(x) if(grepl("A0",x)) "ASD" else if(grepl("Averages",x)||grepl("Standard Deviations",x)) "" else "TYP"))
dfNoNas$Condition<-list
#writetoExcelone(dfNoNas, "/Users/Rgao/Desktop/McGill/Condition CSV files_RG/aggregate_data_filtered_all2_FixationPoint.xlsx", "aggregate_data_incorrect", "table32")
writetoExcelone(dfNoNas, paste(getwd(),"/Condition CSV files_RG/aggregate_data_filtered_all2_FixationPoint.xlsx",sep=""), "aggregate_data_incorrect", "table32")
